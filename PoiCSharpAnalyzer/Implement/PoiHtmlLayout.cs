...
namespace PoiLanguage
{
    ...
    class PoiHtmlLayout
    {
        ...
        public void Generate()
        {
            ...
            switch (type)
            {
                ...
                case PoiLayoutType.Grid:
                    ...
                    htmlcode = string.Format("<div name=\"{2}\" class=\"{3}\" style=\"{0}\">\r\n{1}</div>\r\n",
                        MakeStyleAbso(0, 0, 100, 100, "%"), htmlcode, name, GetProperty("class"));
                    break;
                ...
                case PoiLayoutType.Panel:
                    ...
                    htmlcode = string.Format("<div name=\"{2}\" class=\"{3}\" style=\"{0}\">\r\n{1}</div>\r\n",
                        MakeStyleFlow(), htmlcode, name, GetProperty("class"));
                    break;
                ...
                case PoiLayoutType.Group:
                    ...
                    for (int i = 0; i < cntcol;)
                    {
                        if (ele_list[i] == null)
                        {
                            htmlcode += string.Format("<td name=\"{0}_{1}\"></td>\r\n", name, i);
                            i++; continue;
                        }
                        htmlcode += string.Format("<td colspan=\"{0}\" name=\"{2}_{3}\">\r\n{1}</td>\r\n", ele_list[i].rect.Width, ele_list[i].layout.htmlcode, name, i);
                ...
                case PoiLayoutType.Text: // format=("/biu-+_^><@...")
                    ...
                    else
                    {
                        ...
                        htmlcode = string.Format("<span name=\"{1}\" class=\"{2}\">\r\n{0}\r\n</span>\r\n", htmlcode, name, GetProperty("class"));
                    }
                    break;
                ...
                case PoiLayoutType.Table:
                    ...
                    prefix = string.Format("<table name=\"{0}\" class=\"{1}\">\r\n", name, GetProperty("class"));
                    suffix = "</table>";
                    ...
                    for (int i = 1; i <= cntcol;)
                    {
                        if (table[0, i] == null)
                        {
                            thead += string.Format("<th name=\"{0}_{1}_{2}\"></th>\r\n", name, 0, i);
                            i++; continue;
                        }
                        thead += string.Format("<th colspan=\"{0}\" name=\"{2}_{3}_{4}\">{1}</th>\r\n", table[0, i].rect.Width, table[0, i].layout.htmlcode, name, 0, i);
                    ...
                    for (int i = 1; i <= cntrow; i++)
                    {
                        ...
                        for (int j = 1; j <= cntcol; j++)
                        {
                            ...
                            if (table[i, j] == null)
                            {
                                tbody += string.Format("<td name=\"{0}_{1}_{2}\"></td>\r\n", name, i, j);
                                continue;
                            }
                            tbody += string.Format("<td rowspan=\"{0}\" colspan=\"{1}\" name=\"{3}_{4}_{5}\">{2}</td\r\n>", table[i, j].rect.Height, table[i, j].rect.Width, table[i, j].layout.htmlcode, name, i, j);
                ...
                case PoiLayoutType.Input:
                    ...
                    htmlcode = string.Format("<input name=\"{0}\" value=\"{1}\" class=\"{2}\" {3} />",
                        name, GetProperty("text"), GetProperty("class"), input);
                    break;
                ...
                case PoiLayoutType.Image:
                    htmlcode = string.Format("<img src=\"{0}\" alt=\"{1}\" name=\"{2}\" class=\"{3}\" />\r\n",
                        GetProperty("url"), GetProperty("text"), name, GetProperty("class"));
                    break;
                ...
                case PoiLayoutType.Part:
                    ...
                    htmlcode = string.Format("<{0} name=\"{2}\" class=\"{3}\">\r\n{1}</{0}>\r\n",
                        GetProperty("type"), htmlcode, name, GetProperty("class"));
                    break;

                case PoiLayoutType.Single:
                    htmlcode = string.Format("<{0} name=\"{1}\" class=\"{2}\" />\r\n",
                        GetProperty("type"), name, GetProperty("class"));
                    break;
                ...
                case PoiLayoutType.Button:
                    ...
                    htmlcode = string.Format("<button name=\"{1}\" class=\"{2}\" {3}>\r\n{0}</button>\r\n",
                    htmlcode, name, GetProperty("class"), btn);
                    break;
                ...
                case PoiLayoutType.Textarea:
                    ...
                    htmlcode = string.Format("<textarea name=\"{1}\" class=\"{2}\" {3}>\r\n{0}</textarea>\r\n",
                    htmlcode, name, GetProperty("class"), area);
                    break;
                default:
                    break;
            }
        ...
        // 动态添加子元素
        public string Append(List<string> data)
        {
            if (!Map.ContainsKey(data[0]))
                throw new PoiAnalyzeException("Struct.add: adding not existing element " + data[0]);
            PoiHtmlLayout child = Map[data[0]];
        ...
        public PoiObject Solve(string operation, Node dataNode)
        {
            ...
            switch (operation) // 静态操作按照C#规则翻译，data应该为Literal构成的Pair；动态操作翻译为JS，直接生成操作data的代码。
            {
                ...
                case "append":
                    data = ParseStatic(dataNode);
                    return new PoiObject(PoiObjectType.String, this.Append(data));
                case "show":

...
namespace PoiLanguage
{
    class PoiBasicAnalyzer : PoiAnalyzer
    {
        ...
        {
            PoiInfo.AddValuePos(node, new PoiObject(PoiObjectType.String, node.GetImage(), PoiType.StringToVariableType(node.GetImage())));
            return node;
        }
        ...
        {
            PoiInfo.AddValuePos(node, new PoiObject(PoiObjectType.String, "var", PoiVariableType.String));
            return node;
        }
        ...
        public override Node ExitPoiSource(Production node)
        {
            ...
            try
            {
                ...
                jsHeader = sr.ReadToEnd().ToString();
                sr.Close();
            }
            catch (Exception)
            {
            ...
            String code = string.Format("{0}\r\n$(document).ready(function(){{\r\n{1}\r\n}});", jsHeader, MergeChildList(node).ToString());
            ...
            StreamWriter writer = new StreamWriter("views/js/poi~.js");
            writer.Write(code);
            writer.Close();
            return node;
        }
            ...
            else if (type == "ContainerType")
            {
                ...
                if (containerNode.Name == "ArrayContainer")
                {
                    ...
                    for (int i = 1; i <= num; i++)
                    {
                        ...
                        if (i == num)
                            result += " " + PoiType.GetDefaultInitializer((containerNode.GetChildAt(2).GetValue(0) as PoiObject).VariableType) + ";\r\n";
                        else
  ...
  */
        ...
        public override Node ExitArrayContainer(Production node)
        {
            ...
            PoiInfo.AddValuePos(node, new PoiObject(PoiObjectType.Array, array, PoiVariableType.Array));
            return node;
        }
        ...
        {
            PoiObject expr = MergeChildList(node);
            expr.VariableType = PoiVariableType.Event;
            PoiInfo.AddValuePos(node, expr);
            return node;
        }
